...
ethnic <- dat_pc_tmp %>% 
  select(
    ...
    black_r, black_p, 
    asian_r, asian_p, 
    raceother_r, raceother_p
  ) %>% 
  as.data.frame() %>% 
  Calc_np(an_only = T)


LP_dat_ET <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_ET,
             # order_dat = tri_ord,
             axis_lab = "Ethnicity (%)",
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Ethnicity.png"),
             stacked = T)
...
LP_dat_aura <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_aura,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Aura.png"),
             stacked = T,
             single = T,
             rmv_leg = T)
...
severity <- dat_pc_tmp %>% 
  select(
    ...
    N = migrane_n,
    severe_r, severe_p, 
    moderate_r, moderate_p, 
    mild_r, mild_p
  ) %>% 
  as.data.frame() %>% 
  Calc_np(an_only = T)

LP_dat_SEV <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_SEV,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Migraine severity.png"),
             stacked = T)
...
DUR <- dat_pc_tmp %>% 
  ...
  as.data.frame() %>% 
  Prep_cont(prefix = "dur", clean = F) %>%
  mutate(est_type_dur = "Mean") #Just 'hacking' this so it works properly for this function

LP_dat_dur <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
  baseline.vars = DUR[, 1, drop = F]
)
...
lorenzi_plot(dat = LP_dat_dur,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Duration of migraine.png"),
             lowl = 0,
             upl = 40)
...
#   ) %>% 
#   as.data.frame() %>% 
#   Prep_cont(prefix = "tfd", clean = F) %>%
#   mutate(est_type_tfd = "Mean") #Just 'hacking' this so it works properly for this function
# 
# LP_dat_tfd <- prepare.baseline.data(
#   id.var = dat_pc_tmp$trial, 
#   trt.var = dat_pc_tmp$focus_tr, 
#   n.var = dat_pc_tmp$n,
#   baseline.vars = TFD[, 1, drop = F]
# )
# names(LP_dat_dur)[3] <- "var"
# 
...
#              out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Time from diagnosis.png"),
#              lowl = 0,
#              upl = 30)
...
APM <- dat_pc_tmp %>% 
  mutate(
    attacks_n = n, 
    attacks_median = attacks_est, 
    attacks_mean = NA_real_,
    attacks_sd = attacks_disp, 
    attacks_se = NA_real_, 
    attacks_95l = NA_real_, 
    attacks_95u = NA_real_,
    attacks_iqrl = NA_real_, 
    attacks_iqru = NA_real_
  ) %>% 
  as.data.frame() %>% 
  Prep_cont(prefix = "attacks", clean = F) %>%
  mutate(est_type_attacks = "Mean") #Just 'hacking' this so it works properly for this function

LP_dat_attacks <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
  baseline.vars = APM[, 1, drop = F]
)
...
lorenzi_plot(dat = LP_dat_attacks,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Attacks per month.png"),
             lowl = 0,
             upl = 10,
             breaks = 1)
...
LP_dat_sympnausea <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_sympnausea,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Nausea symptoms.png"),
             stacked = T,
             single = T,
             rmv_leg = T)
...
LP_dat_sympvomit <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_sympvomit,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Vomiting symptoms.png"),
             stacked = T,
             single = T,
             rmv_leg = T)
...
LP_dat_sympphono <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_sympphono,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Phonophobia symptoms.png"),
             stacked = T,
             single = T,
             rmv_leg = T)
...
LP_dat_sympphoto <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_sympphoto,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - Photophobia symptoms.png"),
             stacked = T,
             single = T,
             rmv_leg = T)
...
LP_dat_sympnone <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_sympnone,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - No symptoms.png"),
             stacked = T,
             single = T,
             rmv_leg = T)
...
mbs <- dat_pc_tmp %>% 
  select(
    ...
    treatment,
    N = mbs_n,
    mbsnausea_r, mbsnausea_p, 
    mbsphono_r, mbsphono_p, 
    mbsphoto_r, mbsphoto_p
  ) %>% 
  as.data.frame() %>% 
  Calc_np(an_only = T)

LP_dat_MBS <- prepare.baseline.data(
  id.var = dat_pc_tmp$trial, 
  trt.var = dat_pc_tmp$focus_tr, 
  n.var = dat_pc_tmp$n,
...
lorenzi_plot(dat = LP_dat_MBS,
             ...
             out_path = paste0(out_dir, "Lorenzi plots/Lorenzi plot - MBS.png"),
             stacked = T)
